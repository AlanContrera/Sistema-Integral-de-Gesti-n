...
## 5. Seguridad y Gestión de Secretos (PROHIBICIÓN TOTAL DE KEYS Y CREDENCIALES)
- **CERO KEYS O SECRETOS EN NINGÚN LUGAR DEL REPOSITORIO O COMMITS:** Está estrictamente prohibido subir, commitear o versionar cualquier API Key (Telegram, Gemini, OpenAI, etc.), tokens de servicios (Healthchecks, webhooks), contraseñas de bases de datos, credenciales SMTP de correos, llaves privadas o secretos a **ningún lugar del repositorio ni en el historial de Git**.
- **ALCANCE TOTAL (CÓDIGO, DOCUMENTACIÓN Y COMMITS):** Esta prohibición aplica sin excepción tanto al código fuente (`.py`, `.js`, `.jsx`, `.ts`, `.tsx`, `.json`, `.yml`), como a archivos de documentación (`.md`, manuales, walkthroughs, notas de Obsidian, READMEs, scripts bash de ejemplo) y mensajes de commit.
- **USO ESTRICTO DE PLACEHOLDERS EN GUÍAS Y EJEMPLOS:** En manuales, snippets de terminal (`cat << EOF`) o guías donde se ilustre cómo inyectar variables, está **terminantemente prohibido** copiar y pegar valores reales. Se deben utilizar obligatoria y exclusivamente variables neutras o placeholders ficticios (ej. `TELEGRAM_BOT_TOKEN=tu_telegram_bot_token_aqui`, `<API_KEY>`, `https://hc-ping.com/tu-uuid-aqui`).
- **USO OBLIGATORIO DE VARIABLES DE ENTORNO (`.env`):** Todo secreto o credencial debe residir exclusivamente en el archivo local `.env` del entorno respectivo (el cual debe estar verificado y protegido dentro de `.gitignore`).
...
- **AUDITORÍA PRE-COMMIT / PRE-PUSH OBLIGATORIA:** Antes de proponer o ejecutar cualquier commit o push a Git, es obligatorio auditar el diff completo (`git diff`, `git diff --cached`) para asegurar que no exista ninguna key, token o credencial real en los archivos que se van a registrar.
